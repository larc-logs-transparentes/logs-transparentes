    ...
    _createHashes(nodes) {
        ...
        while (nodes.length > 1) {
            ...
            for (let i = 0; i < nodes.length; i += 2) {
                ...
                for (var ik in combined[1].vote) {
                    if (datavote.get(ik) === undefined) {
                        datavote.set(ik, combined[1].vote.get(ik));
                    }
                    else {
                        datavote.set(ik, datavote.get(ik) + combined[1].vote.get(ik));
                    }
                }
                console.log(buffer_1.Buffer.from(Array.from(datavote)));
                console.log(combined[1]);
                console.log(combined[1].leaf);
                console.log(this.bufferify(combined[1].leaf));
                console.log([buffer_1.Buffer.from(Array.from(datavote)), this.bufferify(combined[0].leaf), this.bufferify(combined[1].leaf)]);
                dataleaf = buffer_1.Buffer.concat([buffer_1.Buffer.from(Array.from(datavote)), this.bufferify(combined[0].leaf), this.bufferify(combined[1].leaf)], 3);
